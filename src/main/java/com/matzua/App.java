...
//        eventRegistrar.register("player.move.backward", (boolean key) -> key ? );
        inputManager.bind((int) 'W', "forward");
        inputManager.bind((int) 'S', "backward");
        inputManager.bind((int) 'A', "left");
        inputManager.bind((int) 'D', "right");
...
//        stateManager.put("right", camera -> camera.x += 10);
    ...
    public void draw() {
        ...
        Function.<InputManager>identity()
            .andThen(InputManager::translate)
            //.andThen(actionManager::dispatch)
            .apply(inputManager);
